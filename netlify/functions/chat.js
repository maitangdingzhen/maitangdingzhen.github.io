...
// Netlify Serverless Function — 代理前端请求到 DeepSeek API
...
export async function handler(event) {
  ...
  if (!DEEPSEEK_API_KEY) {
    return { statusCode: 500, headers: corsHeaders(), body: JSON.stringify({ error: '服务配置错误：API Key 未设置' }) };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorMap = { 401: 'API 密钥无效', 402: 'API 余额不足', 429: '请求过于频繁，请稍后重试', 500: 'AI 服务暂时不可用', 503: 'AI 服务繁忙' };
      return {
        statusCode: status,
        headers: corsHeaders(),
        body: JSON.stringify({ error: errorMap[status] || '服务异常，请稍后重试', code: 'API_ERROR' })
      };
    }
    ...
    return {
      statusCode: 500,
      headers: corsHeaders(),
      body: JSON.stringify({ error: '网络连接失败，请检查后重试', code: 'NETWORK_ERROR' })
    };
  }
}
